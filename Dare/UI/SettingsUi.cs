...
using Dare.API.Data;
using Dare.API.Data.Comparer;
using Dare.API.Routes;
...
using Dare.Utils;
using Dare.WebAPI;
using Dare.WebAPI.Files;
using Dare.WebAPI.Files.Models;
using Dare.WebAPI.SignalR.Utils;
...
public class SettingsUi : WindowMediatorSubscriberBase
{
    ...
    private readonly DareConfigService _configService;
    ...
    public SettingsUi(ILogger<SettingsUi> logger,
        UiSharedService uiShared, DareConfigService configService,
        PairManager pairManager,
        ServerConfigurationManager serverConfigurationManager,
        PlayerPerformanceConfigService playerPerformanceConfigService,
        DareMediator mediator, PerformanceCollectorService performanceCollector,
        FileUploadManager fileTransferManager,
        FileTransferOrchestrator fileTransferOrchestrator,
        FileCacheManager fileCacheManager,
        FileCompactor fileCompactor, ApiController apiController,
        IpcManager ipcManager, CacheMonitor cacheMonitor,
        DalamudUtilService dalamudUtilService, HttpClient httpClient) : base(logger, mediator, "Dare Synchronos Settings", performanceCollector)
    {
    ...
    private void DrawBlockedTransfers()
    {
        ...
        UiSharedService.ColorTextWrapped("Files that you attempted to upload or download that were forbidden to be transferred by their creators will appear here. " +
                             ...
                             "Ask your paired friend to send you the mod in question through other means, acquire the mod yourself or pester the mod creator to allow it to be sent over Dare.",
    ...
    private void DrawFileStorageSettings()
    {
        ...
        if (_uiShared.IconTextButton(FontAwesomeIcon.Running, "Open Dare Character Data Hub"))
        {
        ...
        UiSharedService.TextWrapped("Dare stores downloaded files from paired people permanently. This is to improve loading performance and requiring less downloads. " +
        ...
        ImGui.AlignTextToFramePadding();
        ImGui.TextUnformatted("Monitoring Dare Storage Folder: " + (_cacheMonitor.DareWatcher?.Path ?? "Not monitoring"));
        if (string.IsNullOrEmpty(_cacheMonitor.DareWatcher?.Path))
        {
            ImGui.SameLine();
            using var id = ImRaii.PushId("mareMonitor");
            if (_uiShared.IconTextButton(FontAwesomeIcon.ArrowsToCircle, "Try to reinitialize Monitor"))
            {
                _cacheMonitor.StartDareWatcher(_configService.Current.CacheFolder);
            }
        }
        if (_cacheMonitor.DareWatcher == null || _cacheMonitor.PenumbraWatcher == null)
        {
            ...
            UiSharedService.AttachToolTip("Attempts to resume monitoring for both Penumbra and Dare Storage. "
        ...
        else
        {
            ...
            UiSharedService.AttachToolTip("Stops the monitoring for both Penumbra and Dare Storage. "
                + "Do not stop the monitoring, unless you plan to move the Penumbra and Dare Storage folders, to ensure correct functionality of Dare." + Environment.NewLine
        ...
        {
            UiSharedService.ColorTextWrapped("Hint: To free up space when using Dare consider enabling the File Compactor", ImGuiColors.DalamudYellow);
        }
        ...
        if (!_fileCompactor.MassCompactRunning)
        {
            ...
            UiSharedService.AttachToolTip("This will run compression on all files in your current Dare Storage." + Environment.NewLine
            ...
            UiSharedService.AttachToolTip("This will run decompression on all files in your current Dare Storage.");
        }
        else
        {
        ...
        ImGui.Separator();
        UiSharedService.TextWrapped("File Storage validation can make sure that all files in your local Dare Storage are valid. " +
        ...
        UiSharedService.AttachToolTip("You normally do not need to do this. THIS IS NOT SOMETHING YOU SHOULD BE DOING TO TRY TO FIX SYNC ISSUES." + Environment.NewLine
            ...
            + "Dares storage is self-clearing and will not surpass the limit you have set it to." + Environment.NewLine
    ...
    private void DrawGeneral()
    {
        ...
        _uiShared.DrawHelpText("This will add Dare related right click menu entries in the game UI on paired players.");
        ...
        _uiShared.DrawHelpText("This will add Dare connection status and visible pair count in the Server Info Bar.\nYou can further configure this through your Dalamud Settings.");
        ...
        if (ImGui.Checkbox("Show Dare Profiles on Hover", ref showProfiles))
        {
    ...
    private void DrawPerformance()
    {
        ...
        _uiShared.DrawHelpText("Will show a performance indicator when players exceed defined thresholds in Dares UI." + Environment.NewLine + "Will use warning thresholds.");
        ...
        _uiShared.DrawHelpText("Dare will print a warning in chat once per session of meeting those people. Will not warn on players with preferred permissions.");
        using (ImRaii.Disabled(!warnOnExceedingThresholds && !showPerformanceIndicator))
        {
            ...
            _uiShared.DrawHelpText("Dare will also print warnings and show performance indicator for players where you enabled preferred permissions. If warning in general is disabled, this will not produce any warnings.");
        }
    ...
    private void DrawServerConfiguration()
    {
        ...
        if (ImGui.BeginTabBar("serverTabBar"))
        {
            if (ImGui.BeginTabItem("Character Management"))
            {
                ...
                {
                    UiSharedService.ColorTextWrapped("Characters listed here will automatically connect to the selected Dare service with the settings as provided below." +
                    ...
                    foreach (var item in selectedServer.Authentications.ToList())
                    {
                        ...
                        if (ImGui.Checkbox("Automatically login to Dare", ref isAutoLogin))
                        {
                            item.AutoLogin = isAutoLogin;
                            _serverConfigurationManager.Save();
                        }
                        _uiShared.DrawHelpText("When enabled and logging into this character in XIV, Dare will automatically connect to the current service.");
            ...
            if (ImGui.BeginTabItem("Service Configuration"))
            {
                ...
                if (_dalamudUtilService.IsWine)
                {
                    ...
                    _uiShared.DrawHelpText("On wine, Dare will automatically fall back to ServerSentEvents/LongPolling, even if WebSockets is selected. "
    ...
    private async Task<(bool Success, bool partialSuccess, string Result)> ConvertSecretKeysToUIDs(ServerStorage serverStorage, CancellationToken token)
    {
        ...
        var oauthCheckUri = DareAuth.GetUIDsBasedOnSecretKeyFullPath(new Uri(baseUri));
    ...
    private void DrawSettingsContent()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("Dare Synchronos Discord"))
        {
