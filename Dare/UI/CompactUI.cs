...
public class CompactUi : WindowMediatorSubscriberBase
{
    ...
    public CompactUi(ILogger<CompactUi> logger, UiSharedService uiShared, DareConfigService configService, ApiController apiController, PairManager pairManager,
        ServerConfigurationManager serverManager, DareMediator mediator, FileUploadManager fileTransferManager,
        TagHandler tagHandler, DrawEntityFactory drawEntityFactory, SelectTagForPairUi selectTagForPairUi, SelectPairForTagUi selectPairForTagUi,
        PerformanceCollectorService performanceCollectorService, IpcManager ipcManager)
        : base(logger, mediator, "###DareSynchronosMainUI", performanceCollectorService)
    {
        ...
        TitleBarButtons = new()
        {
            ...
            new TitleBarButton()
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text("Open Dare Event Viewer");
                    ImGui.EndTooltip();
                }
            }
        };
        ...
        WindowName = $"Dare Synchronos {dev} ({ver.Major}.{ver.Minor}.{ver.Build})###DareSynchronosMainUI";
        Toggle();
#else
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        WindowName = "Dare Synchronos " + ver.Major + "." + ver.Minor + "." + ver.Build + "###DareSynchronosMainUI";
#endif
    ...
    protected override void DrawInternal()
    {
        ...
        if (!_apiController.IsCurrentVersion)
        {
            ...
            UiSharedService.ColorTextWrapped($"Your Dare Synchronos installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
                $"It is highly recommended to keep Dare Synchronos up to date. Open /xlplugins and update the plugin.", ImGuiColors.DalamudRed);
        }
        ...
        if (!_ipcManager.Initialized)
        {
            ...
            UiSharedService.ColorTextWrapped($"One or more Plugins essential for Dare operation are unavailable. Enable or update following plugins:", ImGuiColors.DalamudRed);
    ...
    private string GetServerError()
    {
        return _apiController.ServerState switch
        {
            ...
            ServerState.Disconnected => "You are currently disconnected from the Dare Synchronos server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Dare Synchronos server is currently offline.",
            ...
            ServerState.NoAutoLogon => "This character has automatic login into Dare disabled. Press the connect button to connect to Dare.",
            _ => string.Empty
        };
    }
