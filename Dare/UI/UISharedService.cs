...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    public UiSharedService(ILogger<UiSharedService> logger, IpcManager ipcManager, ApiController apiController,
        ...
        DareConfigService configService, DalamudUtilService dalamudUtil, IDalamudPluginInterface pluginInterface,
        ITextureProvider textureProvider,
        Dalamud.Localization localization,
        ServerConfigurationManager serverManager, TokenProvider tokenProvider, DareMediator mediator) : base(logger, mediator)
    {
    ...
    {
        ColorTextWrapped("Note: The storage folder should be somewhere close to root (i.e. C:\\DareStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.", ImGuiColors.DalamudYellow);
        ...
        ImGui.SameLine();
        using (ImRaii.Disabled(_cacheMonitor.DareWatcher != null))
        {
            if (IconButton(FontAwesomeIcon.Folder))
            {
                FileDialogManager.OpenFolderDialog("Pick Dare Synchronos Storage Folder", (success, path) =>
                {
                    if (!success) return;
                    ...
                    {
                        _cacheDirectoryHasOtherFilesThanCache = true;
                        Logger.LogWarning("Found folders in {path} not belonging to Dare: {dirs}", path, string.Join(", ", dirs));
                    }
                        ...
                        && _cacheDirectoryIsValidPath)
                    {
                        ...
                        _configService.Save();
                        _cacheMonitor.StartDareWatcher(path);
                        _cacheMonitor.InvokeScan();
                    }
                }, _dalamudUtil.IsWine ? @"Z:\" : @"C:\");
            }
        }
        if (_cacheMonitor.DareWatcher != null)
        {
        ...
        {
            ColorTextWrapped("Your selected directory has files or directories inside that are not Dare related. Use an empty directory or a previous Dare storage directory only.", ImGuiColors.DalamudRed);
        }
        ...
        DrawHelpText("The storage is automatically governed by Dare. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
    ...
    public bool DrawOtherPluginState()
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudRed, "You need to install both Penumbra and Glamourer and keep them up to date to use Dare Synchronos.");
            return false;
        }
