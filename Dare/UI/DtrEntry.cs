...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    private readonly ConfigurationServiceBase<DareConfig> _configService;
    ...
    public DtrEntry(ILogger<DtrEntry> logger, IDtrBar dtrBar, ConfigurationServiceBase<DareConfig> configService, DareMediator mareMediator, PairManager pairManager, ApiController apiController)
    {
        _logger = logger;
        _dtrBar = dtrBar;
    ...
    private IDtrBarEntry CreateEntry()
    {
        ...
        var entry = _dtrBar.Get("Dare Synchronos");
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                tooltip = $"Dare Synchronos: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
                colors = _configService.Current.DtrColorsPairsInRange;
            }
            else
            {
                tooltip = "Dare Synchronos: Connected";
        ...
        {
            text = "\uE044 \uE04C";
            tooltip = "Dare Synchronos: Not Connected";
