...
public partial class IntroUi : WindowMediatorSubscriberBase
{
    ...
    public IntroUi(ILogger<IntroUi> logger, UiSharedService uiShared, DareConfigService configService,
        CacheMonitor fileCacheManager, ServerConfigurationManager serverConfigurationManager, DareMediator mareMediator,
        PerformanceCollectorService performanceCollectorService, DalamudUtilService dalamudUtilService) : base(logger, mareMediator, "Dare Synchronos Setup", performanceCollectorService)
    {
        _uiShared = uiShared;
    ...
    protected override void DrawInternal()
    {
        ...
        {
            _uiShared.BigText("Welcome to Dare");
            ImGui.Separator();
            UiSharedService.TextWrapped("Dare is a plugin that will replicate your full current character state including all Penumbra mods to other paired Dare users. " +
                     ...
                     || !Directory.Exists(_configService.Current.CacheFolder)))
        {
            ...
            {
                UiSharedService.TextWrapped("To not unnecessary download files already present on your computer, Dare will have to scan your Penumbra mod directory. " +
                                     "Additionally, a local storage folder must be set where Dare will download other character files to. " +
                                     "Once the storage folder is set and the scan complete, this page will automatically forward to registration at a service.");
                UiSharedService.TextWrapped("Note: The initial scan, depending on the amount of mods you have, might take a while. Please wait until it is completed.");
                UiSharedService.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.csv of Dare in the Plugin Configurations folder of Dalamud. " +
            ...
            if (!_dalamudUtilService.IsWine)
            {
                ...
                UiSharedService.ColorTextWrapped("The File Compactor can save a tremendeous amount of space on the hard disk for downloads through Dare. It will incur a minor CPU penalty on download but can speed up " +
                    "loading of other characters. It is recommended to keep it enabled. You can change this setting later anytime in the Dare settings.", ImGuiColors.DalamudYellow);
            }
        }
        else if (!_uiShared.ApiController.ServerAlive)
        {
            ...
            ImGui.Separator();
            UiSharedService.TextWrapped("To be able to use Dare you will have to register an account.");
            UiSharedService.TextWrapped("For the official Dare Servers the account creation will be handled on the official Dare Discord. Due to security risks for the server, there is no way to handle this sensibly otherwise.");
            ...
            if (ImGui.Button("Join the Dare Discord"))
            {
