...
using NotificationType = Dare.DareConfiguration.Models.NotificationType;
...
public class NotificationService : DisposableMediatorSubscriberBase, IHostedService
{
    ...
    private readonly DareConfigService _configurationService;

    public NotificationService(ILogger<NotificationService> logger, DareMediator mediator,
        DalamudUtilService dalamudUtilService,
        INotificationManager notificationManager,
        IChatGui chatGui, DareConfigService configurationService) : base(logger, mediator)
    {
    ...
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Dare] Error: " + message);
        _chatGui.PrintError(se.BuiltString);
    }

    private void PrintInfoChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Dare] Info: ").AddItalics(message ?? string.Empty);
        _chatGui.Print(se.BuiltString);
    }

    private void PrintWarnChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Dare] ").AddUiForeground("Warning: " + (message ?? string.Empty), 31).AddUiForegroundOff();
        _chatGui.Print(se.BuiltString);
    }
