...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    private readonly DareCharaFileDataFactory _mareCharaFileDataFactory;
    ...
    public Task<(DareCharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
            ...
            var loadedCharaFile = DareCharaFileHeader.FromBinaryReader(filePath, reader);

            _logger.LogInformation("Read Dare Chara File");
    ...
    public Dictionary<string, string> McdfExtractFiles(DareCharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        DareCharaFileHeader.AdvanceReaderToData(reader);
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            DareCharaFileHeader output = new(DareCharaFileHeader.CurrentVersion, mareCharaFileData);
        ...
        {
            _logger.LogError(ex, "Failure Saving Dare Chara File, deleting output");
