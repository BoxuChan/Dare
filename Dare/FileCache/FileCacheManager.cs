...
public sealed class FileCacheManager : IHostedService
{
    ...
    public FileCacheManager(ILogger<FileCacheManager> logger, IpcManager ipcManager, DareConfigService configService, DareMediator mareMediator)
    {
        _logger = logger;
        ...
        if (File.Exists(_csvPath))
        {
            if (!_ipcManager.Penumbra.APIAvailable || string.IsNullOrEmpty(_ipcManager.Penumbra.ModDirectory))
            {
                _mareMediator.Publish(new NotificationMessage("Penumbra not connected",
                    "Could not load local file cache data. Penumbra is not connected or not properly set up. Please enable and/or configure Penumbra properly to use Dare. After, reload Dare in the Plugin installer.",
                    DareConfiguration.Models.NotificationType.Error));
            }
