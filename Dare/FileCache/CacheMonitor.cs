...
public sealed class CacheMonitor : DisposableMediatorSubscriberBase
{
    ...
    public CacheMonitor(ILogger<CacheMonitor> logger, IpcManager ipcManager, DareConfigService configService,
        FileCacheManager fileDbManager, DareMediator mediator, PerformanceCollectorService performanceCollector, DalamudUtilService dalamudUtil,
        FileCompactor fileCompactor) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<PenumbraInitializedMessage>(this, (_) =>
        {
            ...
            StartDareWatcher(configService.Current.CacheFolder);
            InvokeScan();
        });
    ...
    {
        Logger.LogInformation("Stopping monitoring of Penumbra and Dare storage folders");
        DareWatcher?.Dispose();
    ...
    public void StartDareWatcher(string? marePath)
    {
        DareWatcher?.Dispose();
        if (string.IsNullOrEmpty(marePath) || !Directory.Exists(marePath))
        {
            DareWatcher = null;
            Logger.LogWarning("Dare file path is not set, cannot start the FSW for Dare.");
            return;
        }
        ...
        Logger.LogInformation("Dare Storage is on NTFS drive: {isNtfs}", StorageisNTFS);

        Logger.LogDebug("Initializing Dare FSW on {path}", marePath);
        DareWatcher = new()
        {
            Path = marePath,
        ...
        DareWatcher.Deleted += DareWatcher_FileChanged;
        DareWatcher.Created += DareWatcher_FileChanged;
        DareWatcher.EnableRaisingEvents = true;
    }

    private void DareWatcher_FileChanged(object sender, FileSystemEventArgs e)
    {
        Logger.LogTrace("Dare FSW: FileChanged: {change} => {path}", e.ChangeType, e.FullPath);
    ...
    public FileSystemWatcher? DareWatcher { get; private set; }

    private async Task DareWatcherExecution()
    {
    ...
    private void FullFileScan(CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(_configService.Current.CacheFolder) || !Directory.Exists(_configService.Current.CacheFolder))
        {
            ...
            Logger.LogWarning("Dare Cache directory is not set or does not exist.");
        }
        ...
        if (!_configService.Current.InitialScanComplete)
        {
            ...
            _configService.Save();
            StartDareWatcher(_configService.Current.CacheFolder);
