...
public class IpcProvider : IHostedService, IMediatorSubscriber
{
    ...
    public DareMediator Mediator { get; init; }

    public IpcProvider(ILogger<IpcProvider> logger, IDalamudPluginInterface pi,
        CharaDataManager charaDataManager, DareMediator mareMediator)
    {
        _logger = logger;
        _pi = pi;
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _loadFileProvider = _pi.GetIpcProvider<string, IGameObject, bool>("DareSynchronos.LoadMcdf");
        _loadFileProvider.RegisterFunc(LoadMcdf);
        _loadFileAsyncProvider = _pi.GetIpcProvider<string, IGameObject, Task<bool>>("DareSynchronos.LoadMcdfAsync");
        _loadFileAsyncProvider.RegisterFunc(LoadMcdfAsync);
        _handledGameAddresses = _pi.GetIpcProvider<List<nint>>("DareSynchronos.GetHandledAddresses");
