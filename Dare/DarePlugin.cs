...
public class DarePlugin : MediatorSubscriberBase, IHostedService
{
    ...
    public DarePlugin(ILogger<DarePlugin> logger, DareConfigService mareConfigService,
        ServerConfigurationManager serverConfigurationManager,
        DalamudUtilService dalamudUtil,
        IServiceScopeFactory serviceScopeFactory, DareMediator mediator) : base(logger, mediator)
    {
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        Logger.LogInformation("Launching {name} {major}.{minor}.{build}", "Dare", version.Major, version.Minor, version.Build);
        Mediator.Publish(new EventMessage(new Services.Events.Event(nameof(DarePlugin), Services.Events.EventSeverity.Informational,
            $"Starting Dare {version.Major}.{version.Minor}.{version.Build}")));
    ...
    public Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        Logger.LogDebug("Halting DarePlugin");
        ...
        try
        {
            ...
            if (_mareConfigService.Current.LogLevel != LogLevel.Information)
            {
                Mediator.Publish(new NotificationMessage("Abnormal Log Level",
                    $"Your log level is set to '{_mareConfigService.Current.LogLevel}' which is not recommended for normal usage. Set it to '{LogLevel.Information}' in \"Dare Settings -> Debug\" unless instructed otherwise.",
                    DareConfiguration.Models.NotificationType.Error, TimeSpan.FromSeconds(15000)));
            }
#endif
        }
        catch (Exception ex)
