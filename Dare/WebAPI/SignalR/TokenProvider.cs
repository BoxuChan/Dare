...
public sealed class TokenProvider : IDisposable, IMediatorSubscriber
{
    ...
    public TokenProvider(ILogger<TokenProvider> logger, ServerConfigurationManager serverManager, DalamudUtilService dalamudUtil, DareMediator mareMediator, HttpClient httpClient)
    {
        _logger = logger;
    ...
    public async Task<string> GetNewToken(bool isRenewal, JwtIdentifier identifier, CancellationToken ct)
    {
        ...
        try
        {
            if (!isRenewal)
            {
                ...
                {
                    tokenUri = DareAuth.AuthFullPath(new Uri(_serverManager.CurrentApiUrl
                ...
                {
                    tokenUri = DareAuth.AuthWithOauthFullPath(new Uri(_serverManager.CurrentApiUrl
                        .Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
                        .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase)));
            ...
            else
            {
                ...
                tokenUri = DareAuth.RenewTokenFullPath(new Uri(_serverManager.CurrentApiUrl
                    .Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
                    .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase)));
        ...
        catch (HttpRequestException ex)
        {
            ...
            {
                if (isRenewal)
                    Mediator.Publish(new NotificationMessage("Error refreshing token", "Your authentication token could not be renewed. Try reconnecting to Dare manually.",
                    NotificationType.Error));
                else
                    Mediator.Publish(new NotificationMessage("Error generating token", "Your authentication token could not be generated. Check Dares Main UI (/dare in chat) to see the error message.",
                    NotificationType.Error));
                Mediator.Publish(new DisconnectedMessage());
                throw new DareAuthFailureException(response);
            }
        ...
        if (tokenTime <= dateTimeMinus10 || tokenTime >= dateTimePlus10)
        {
            ...
            Mediator.Publish(new NotificationMessage("Invalid system clock", "The clock of your computer is invalid. " +
                "Dare will not function properly if the time zone is not set correctly. " +
    ...
    public async Task<bool> TryUpdateOAuth2LoginTokenAsync(ServerStorage currentServer, bool forced = false)
    {
        ...
        var tokenUri = DareAuth.RenewOAuthTokenFullPath(new Uri(currentServer.ServerUri
            .Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
            .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase)));
        HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, tokenUri.ToString());
