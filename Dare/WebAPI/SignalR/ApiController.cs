...
using Dare.API.Data;
using Dare.API.Data.Extensions;
using Dare.API.Dto;
...
public sealed partial class ApiController : DisposableMediatorSubscriberBase, IDareHubClient
{
    ...
    public ApiController(ILogger<ApiController> logger, HubFactory hubFactory, DalamudUtilService dalamudUtil,
        PairManager pairManager, ServerConfigurationManager serverManager, DareMediator mediator,
        TokenProvider tokenProvider, DareConfigService mareConfigService) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<HubClosedMessage>(this, (msg) => DareHubOnClosed(msg.Exception));
        Mediator.Subscribe<HubReconnectedMessage>(this, (msg) => _ = DareHubOnReconnectedAsync());
        Mediator.Subscribe<HubReconnectingMessage>(this, (msg) => DareHubOnReconnecting(msg.Exception));
    ...
    public async Task CreateConnectionsAsync()
    {
        ...
        if (!_serverManager.CurrentServer.UseOAuth2)
        {
            ...
            if (multi)
            {
                ...
                _connectionDto = null;
                Mediator.Publish(new NotificationMessage("Multiple Identical Characters detected", "Your Service configuration has multiple characters with the same name and world set up. Delete the duplicates in the character management to be able to connect to Dare.",
        ...
        while (ServerState is not ServerState.Connected && !token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                catch (DareAuthFailureException ex)
                {
                ...
                if (_connectionDto.ServerVersion != IDareHub.ApiVersion)
                {
                    if (_connectionDto.CurrentClientVersion > currentClientVer)
                    {
                        Mediator.Publish(new NotificationMessage("Client incompatible",
                            ...
                            $"This client version is incompatible and will not be able to connect. Please update your Dare Synchronos client.",
                            NotificationType.Error));
                    }
                ...
                if (_connectionDto.CurrentClientVersion > currentClientVer)
                {
                    Mediator.Publish(new NotificationMessage("Client outdated",
                        $"Your client is outdated ({currentClientVer.Major}.{currentClientVer.Minor}.{currentClientVer.Build}), current is: " +
                        $"{_connectionDto.CurrentClientVersion.Major}.{_connectionDto.CurrentClientVersion.Minor}.{_connectionDto.CurrentClientVersion.Build}. " +
                        $"Please keep your Dare Synchronos client up-to-date.",
                ...
                if (_dalamudUtil.HasModifiedGameFiles)
                {
                    ...
                    if (!_mareConfigService.Current.DebugStopWhining)
                        Mediator.Publish(new NotificationMessage("Modified Game Files detected",
                            ...
                            "Dare Synchronos, Penumbra, and some other plugins assume your FFXIV installation is unmodified in order to work. " +
                ...
                if (_dalamudUtil.IsLodEnabled && !_naggedAboutLod)
                {
                    ...
                    if (!_mareConfigService.Current.DebugStopWhining)
                    {
                        Mediator.Publish(new NotificationMessage("Model LOD is enabled",
                            ...
                            "random crashes when loading in or rendering modded pairs. Disabling LOD has a very low performance impact. Disable LOD while using Dare: " +
    ...
    private void DareHubOnClosed(Exception? arg)
    {
        _healthCheckTokenSource?.Cancel();
    ...
    private async Task DareHubOnReconnectedAsync()
    {
        ServerState = ServerState.Reconnecting;
        try
        {
    ...
    private void DareHubOnReconnecting(Exception? arg)
    {
